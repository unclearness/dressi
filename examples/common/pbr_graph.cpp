...
PbrGBuffer BuildPbrGBuffer(const PbrGeometryLeaves& geom, ImgSize screen) {
    PbrGBuffer g;
    Variable g_uvh = F::Rasterize(geom.clip, geom.uvh, geom.faces, screen);
    g.g_n = F::Rasterize(geom.clip, geom.normal, geom.faces, screen);
    g.g_t = F::Rasterize(geom.clip, geom.tangent, geom.faces, screen);
    g.g_p = F::Rasterize(geom.clip, geom.wpos, geom.faces, screen);
    g.uv = F::Vec2(F::GetX(g_uvh), F::GetY(g_uvh));
    g.mask = F::GetZ(g_uvh);
    // Interpolated handedness stays near +-1 inside a face; +1 at background
    g.handed = F::Sign(F::GetW(g_uvh) + 1e-6f);
    return g;
}

PbrOutputs BuildPbrShade(const PbrGBuffer& gbuf, const PbrMaterialLeaves& mat,
                         const PbrFrameLeaves& frame, const PbrIblMaps& ibl,
                         const Variable& inv_uv, ImgSize screen,
                         bool normal_mapping) {
    const Variable& uv = gbuf.uv;
    const Variable& mask = gbuf.mask;

    // ------------------------- Material fetches -------------------------
    Variable albedo = F::TextureBilinear(mat.albedo, uv, inv_uv);
    Variable mr = F::TextureBilinear(mat.mr, uv, inv_uv);
    Variable rough = F::Clamp(F::GetY(mr), F::Float(0.045f), F::Float(1.f));
    Variable metal = F::GetZ(mr);
    Variable ao = F::GetX(F::TextureBilinear(mat.ao, uv, inv_uv));
    Variable emissive = F::TextureBilinear(mat.emissive, uv, inv_uv);

    // --------------------------- Shading normal --------------------------
    Variable n0 = SafeNormalize(gbuf.g_n);
    Variable n = n0;
    if (normal_mapping) {
        Variable t0 = SafeNormalize(gbuf.g_t - n0 * F::Dot(n0, gbuf.g_t));
        Variable b0 = F::Cross(n0, t0) * gbuf.handed;
        Variable nm =
                F::TextureBilinear(mat.normal_map, uv, inv_uv) * 2.f - 1.f;
        n = SafeNormalize(t0 * F::GetX(nm) + b0 * F::GetY(nm) +
                          n0 * F::GetZ(nm));
    }

    // ------------------------ Common shading terms -----------------------
    Variable v = SafeNormalize(frame.cam_pos - gbuf.g_p);
    ...
    Variable to_light = frame.light_pos - gbuf.g_p;
    ...
    Variable lo = (kd_direct * albedo / kPi + spec_d) * radiance * n_dot_l;
    ...
    Variable ndc_x = F::GetX(sc) * (2.f / float(screen.w)) - 1.f;
    Variable ndc_y = F::GetY(sc) * (2.f / float(screen.h)) - 1.f;
    ...
    out.mapped = out.hdr / (out.hdr + 1.f);
    out.ldr = F::Pow(out.mapped, F::Float(1.f / 2.2f));
    out.mask = mask;
    out.uv = uv;
    out.albedo = albedo;
    out.normal = n;
...
PbrOutputs BuildPbrForward(const PbrGeometryLeaves& geom,
                           ...
                           const Variable& inv_uv, ImgSize screen,
                           bool normal_mapping) {
    return BuildPbrShade(BuildPbrGBuffer(geom, screen), mat, frame, ibl,
                         inv_uv, screen, normal_mapping);
}
