# dressi.torch silhouette-optimization benchmark on the glTF Avocado
# (406v/682f) -- the same workload as scripts/nvdiffrast_bench.py: per
# iteration rasterize + antialias(mask) + MSE + backward to vertex
# positions + Adam step. Run with PYTHONPATH=python.
import json
import os
import sys
import time
...
def main():
    ...
    print(f"avocado: {len(pos_np)} verts, {len(tri_np)} faces  backend=dressi(vulkan)")
    ...
    def render_mask(pos, res):
        ...
        clip = (mvp_t @ posh.t()).t().contiguous()
    ...
    for res in resolutions:
        ...
        print(f"dressi.torch {res}x{res}: {ms:.3f} ms/iter"
