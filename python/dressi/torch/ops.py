...
def rasterize(ctx: RasterizeVulkanContext, pos: torch.Tensor,
    ...
    with ctx.lock:
        ...
        rast_cpu = torch.from_numpy(
            eng.run_and_read_outputs_stacked()).reshape(n, h, w, 4)
    ...
    def forward(fctx, attr, rast, vctx, md, rinfo, train):
        ...
        with vctx.lock:
            ...
            out = torch.from_numpy(
                eng.run_and_read_outputs_stacked()).reshape(n, h, w, -1)
    ...
    def backward(fctx, grad_out):
        ...
        with vctx.lock:
            ...
            if shared_attr:
                ...
                g_attr = torch.from_numpy(eng.run_and_read_grads_stacked(
...
class _AntiAliasFn(torch.autograd.Function):
    ...
    def backward(fctx, grad_out):
        ...
        with vctx.lock:
            ...
            g_color = g_pos = None
            if grad_mask[0]:
                g_color = torch.from_numpy(eng.run_and_read_grads_stacked(
                    [_item("img", i) for i in range(n)])).reshape(
                        n, h, w, -1)
                if grad_mask[1]:
                    g_clip = torch.from_numpy(eng.read_grads_stacked(
                        [_item("clip", i) for i in range(n)])).reshape(
                            n, -1, 4)
            elif grad_mask[1]:
                g_clip = torch.from_numpy(eng.run_and_read_grads_stacked(
...
class _RasterizeSoftFn(torch.autograd.Function):
    ...
    def forward(fctx, pos, vctx, md, resolution, radius_px, peels, train):
        ...
        with vctx.lock:
            ...
            out = torch.from_numpy(
                eng.run_and_read_outputs_stacked()).reshape(
                    n, peels, h, w, 4)
