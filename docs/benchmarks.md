...
## Cross-device — 7 Vulkan devices

The identical backward graph runs across three discrete NVIDIA GPUs, an Intel
iGPU, two phones, and a CPU software rasterizer — to the same quality.
...
- **Adreno 740** — Snapdragon 8 Gen 2, Xiaomi, Android 16 (phone)
- **Mali-G1-Ultra MC12** — MediaTek Dimensity 9500, Xiaomi, Android 16 (phone)
...
| | Adreno 740 (phone) | 0.225 | 193 (136) | 36→8→8 | max err 0.0005 |
...
| | Adreno 740 (phone) | 48.361 | 706 (516) | 232→26→25 | 98.7% |
...
| | Adreno 740 (phone) | 21.572 | 1612 (798) | 802→53→47 | IoU 0.9447 |
...
| | Adreno 740 (phone) | 23.661 | 3502 (1587) | 3962→185→64 | IoU 0.9406 |
...
| | Adreno 740 (phone) | 55.171 | 3786 (1868) | 3593→135→49 | PSNR 20.65 dB |
...
| | Adreno 740 (phone) | 304.1 | 3124 (2041) | 2436→53→38 | PSNR 18.26 dB |
...
| | Adreno 740 (phone) | 17.717 (56 FPS) | 825 (638) | 286→11→5 | 56 FPS |
...
- **Quality is device-independent** — the metric matches across all seven
  devices to within stochastic noise (texture 98.7% everywhere, pbr_envmap
  18.23–18.26 dB, silhouette IoU 0.941–0.955), validating the engine.
- **Packing is device-dependent** — the func count is identical, but each
  ...
  phones in between (Mali →125 / Adreno →135). The render-pass (stage) count
  that costs draw calls stays low everywhere.
- **Mobile spans the same graph** — the phones are ~1-2 orders slower on the
  heavy optimizers but converge to the same quality. The Dimensity 9500 beats
  the Snapdragon 8 Gen 2 on the heavy optimizers yet is slower on light/forward
  paths and pays a much larger first-build warmup — its shader compiler, not
  steady-state throughput, is the bottleneck.
